...
![Pandas](https://img.shields.io/badge/Pandas-Data%20Wrangling-150458?style=flat-square)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-ML-F7931E?style=flat-square)
![Seaborn](https://img.shields.io/badge/Seaborn-Visualisation-4C9BE8?style=flat-square)
...
**Steps performed:**
1. Loaded 7,043 rows × 21 columns from CSV
2. Discovered `TotalCharges` stored as object (text) due to blank entries — converted to numeric using `pd.to_numeric(..., errors='coerce')`
3. Found **11 missing values** in `TotalCharges` (new customers with 0 tenure) — filled with median to preserve all rows
4. Dropped `customerID` — a unique identifier with zero predictive value
5. Encoded target: `Churn` → `Yes=1, No=0` → confirmed **26.54% churn rate**
6. Saved a readable copy (`df_eda`) before encoding for use in charts
7. Applied **one-hot encoding** (`pd.get_dummies`) to all categorical columns → expanded from 20 to **30 features**
8. Performed **stratified 80/20 train-test split** (stratified to preserve class ratio)
...
Grouped bar charts for Gender, Partner status, and Dependents:
...
Histogram + boxplot comparing tenure for churned vs retained customers:
...
Churn rates computed per group using `groupby().mean()`:
...
1. **Feature scaling** — `StandardScaler` normalised all features so MonthlyCharges (0–120 range) doesn't dominate binary 0/1 columns
2. **SMOTE** applied to training data only to balance the class imbalance
...
- **Grouped bar chart** — Accuracy vs AUC vs Recall for all 3 models side-by-side
- **ROC curves** — all 3 plotted together with AUC scores in legend for direct comparison
...
| 5 | **1,186 high-value customers** paying avg $86.15/month left within ~16 months on flexible contracts |
...
1. **Prioritise months 1–12** — deploy proactive outreach, welcome calls, and loyalty perks for all new customers in this window
2. **Offer contract upgrade incentives** to month-to-month customers — moving them to a 1-year contract cuts churn risk from 43% → 11%
3. **Migrate electronic check users** to automatic payment via targeted bill-pay discount campaigns
4. **Deploy the Random Forest model** as a live early warning system — flag customers with predicted churn probability >60% for immediate retention action
5. **Focus retention budget on the 1,186 high-value at-risk segment** — retaining just 30% of them at $86.15/month saves approximately **$365,000 in annual revenue**
...
| Field | Detail |
|-------|--------|
| **Company** | [Saiket Systems](https://www.saiket.in) — ISO 9001:2015 Certified |
| **Domain** | Data Science |
...
🔗 [LinkedIn](https://www.linkedin.com/in/SyedNadim123)
📧 nadimulhaq001@gmail.com

---

*#SaiKetSystemsJourney #SaiKetExperience #FutureWithSaiKet #DataScience #CustomerChurn #MachineLearning #Python #sklearn #saiketsystems*
